...
from gpt_code_ui.lru_cache import LRUCache
...
async def start_snakemq():
    ...
    def on_recv(conn, ident, message):
        ...
        if message["type"] == "status":
            ...
            path = message["path"]
            logger.debug("%s of type %s path %s" % (message["value"], message["type"], path))
            if path is not None and result_cache.get(path) is None:
                result_cache.put(path, [])
            result_list = result_cache.get(path)
    ...
    def send_queued_messages():
        while True:
            if send_queue.qsize() > 0:
                path, message = send_queue.get()
                utils.send_json(messaging,
                                {"type": "execute", "value": message["command"], "path": path},
...
def handle_request(path):
    if request.method == "GET":
        ...
        results += result_cache.get(path, list())
        result_cache.put(path, list())
        return jsonify({"results": results})
    elif request.method == "POST":
        data = request.json

        send_queue.put((path, data))
